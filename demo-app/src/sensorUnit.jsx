...
const SensorUnit = (props) => {
        ...
        switch (name) {
            case "Gyroscope":
                ...
                if (gyro_data.data === "TIMEOUT" || gyro_data.data === "FAILED"){
                    ...
                    sensor_data = String(value_1) + `, ` + String(value_2) + `, ` + String(value_3)
                    console.log(sensor_data)
            ...
            case "Switch":
                ...
                if (switch_status.data === "TIMEOUT" || switch_status.data === "FAILED"){
                    setSensorValue(null)
    ...
    return (
        <button disabled={props.disabled} onClick={() => sensorClicked(props.name)} className="border border-2 border-dark rounded-pill pb-3 mt-2 mb-2" style={{justifyContent:'center'}}>
            <div className="row">
                <p>{displayName(props.name)}</p>                
            </div>
            <div className="row d-inline-flex bg-white ps-3 pe-3 sensor-value" style={{justifyContent:'center'}}>
                {!isLoading
                    ?   <div className="m-1">
                            {sensorValue != null
                                ?   <div>{sensorValue}</div>
                    ...
                    :   <div className="m-1">
                            <div className="spinner-grow spinner-grow-sm m-1 text-primary" role="status"/>
                            <div className="spinner-grow spinner-grow-sm m-1 text-primary" role="status"/>
                            <div className="spinner-grow spinner-grow-sm m-1 text-primary" role="status"/>
                        </div>
                }                 
